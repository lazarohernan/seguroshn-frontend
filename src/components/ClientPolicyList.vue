...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div class="flex-1 min-h-0 flex flex-col gap-3">
      <h3 class="text-base font-semibold text-text m-0">Pólizas Activas</h3>

      <!-- Tabla de Pólizas -->
      <div v-if="policies.length > 0" class="flex-1 min-h-0 overflow-auto border border-input-border rounded-xl bg-input-bg">
        <table class="w-full border-separate border-spacing-0">
          <thead class="sticky top-0 z-[1] bg-input-bg">
            <tr>
              ...
              <th class="py-2 px-3 text-xs font-semibold text-text text-left uppercase tracking-wider border-b-2 border-input-border">Póliza</th>
              <th class="py-2 px-3 text-xs font-semibold text-text text-left uppercase tracking-wider border-b-2 border-input-border">Plazo</th>
              <th class="py-2 px-3 text-xs font-semibold text-text text-left uppercase tracking-wider border-b-2 border-input-border">Estado</th>
              <th class="py-2 px-3 text-xs font-semibold text-text text-left uppercase tracking-wider border-b-2 border-input-border"></th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="policy in policies" :key="policy.id" class="hover:bg-background">
              <td class="py-2 px-3 text-sm text-text border-b border-input-border">{{ policy.number }}</td>
              <td class="py-2 px-3 text-sm text-text border-b border-input-border">{{ policy.name }}</td>
              <td class="py-2 px-3 text-sm text-text border-b border-input-border">{{ policy.term }} meses</td>
              ...
              <td class="py-2 px-3 text-sm text-text border-b border-input-border">
                <div class="flex gap-2">
                  <button 
                    ...
                    title="Ver detalles"
                    @click="emit('viewPolicy', policy.id)"
                  >
                    <Eye class="w-4 h-4" />
                  </button>
                  <button 
                    class="p-1.5 rounded-lg border-none bg-background text-text transition-all duration-300 hover:bg-primary hover:text-white hover:-translate-y-0.5 flex items-center justify-center"
                    title="Ver pagos"
                    @click="emit('viewPayments', policy.id)"
                  >
      ...
      <div v-else class="flex-1 flex items-center justify-center p-8 bg-input-bg border border-input-border rounded-xl">
